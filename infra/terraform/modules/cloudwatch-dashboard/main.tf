...
resource "aws_cloudwatch_dashboard" "main" {
  ...
  dashboard_body = jsonencode({
    widgets = [
      ...
      # ── ALB request count (only if ALB exists) ────────────────────────────
      {
        type   = "metric"
        x      = 0
        y      = 6
      ...
      # ── ALB 5xx error rate (only if ALB exists) ──────────────────────────
      {
        type   = "metric"
        x      = 8
        y      = 6
      ...
      # ── RDS free storage space (only if RDS instance exists) ──────────────
      {
        type   = "metric"
        x      = 16
        y      = 6
        ...
        properties = {
          ...
          view   = "timeSeries"
          region = data.aws_region.current.name
          metrics = var.rds_instance_id != "" && var.rds_instance_id != "disabled-rds" ? [
            [
              "AWS/RDS",
              "FreeStorageSpace",
